...
import { View, TextInput, Text, FlatList, Pressable, SafeAreaView, KeyboardAvoidingView, ScrollView } from 'react-native';
...
const Messaging = ({ route, navigation }) => {
  ...
  return (
    <SafeAreaView style={stylesChat.messagingscreen}>
      <View style={[stylesChat.messagingscreen, { paddingVertical: 15, paddingHorizontal: 10 }]}>
        <ScrollView ref={(scrollview) => setScrollRef(scrollview)}>
          {inMsg && Object.keys(inMsg).length !== 0 ? (
            inMsg.map((val, idx) => {
      ...
      <KeyboardAvoidingView>
        <View style={stylesChat.messaginginputContainer}>
          <TextInput value={message} style={stylesChat.messaginginput} onChangeText={(value) => setMessage(value)} />
          <Pressable style={stylesChat.messagingbuttonContainer} onPress={handleNewMessage}>
            <View>
