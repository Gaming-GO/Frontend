...
export default function AddTab({ navigation }) {
  ...
  const addProduct = () => {
    ...
    dispatch(postDevice(input, access_token)).then((_) => navigation.navigate('AllTab'));
    // console.log(categoryId);
    // console.log(input);
  };
  return (
      ...
      <View style={styles.container}>
        ...
        <View style={styles.inputView}>
          <Menu
            visible={visible}
            anchor={
              <Text style={{ fontWeight: 'bold', paddingTop: 10 }} onPress={showMenu}>
                {pick ? pick : 'pick specs range'}
              </Text>
            }
            onRequestClose={hideMenu}
            style={{}}
          >
            <MenuItem onPress={() => handleMenuItemSelection('High-end')} data-my-value={'High-end'}>
              High-end
            </MenuItem>
            <MenuItem onPress={() => handleMenuItemSelection('Mid-end')} value="Mid-end">
              Mid-end
            </MenuItem>
            <MenuItem onPress={() => handleMenuItemSelection('Low-end')} value="Low-end">
              Low-end
            </MenuItem>
          </Menu>
        </View>
        <View style={styles.inputView}>
          <TextInput style={styles.TextInput} placeholder="Price" keyboardType="numeric" placeholderTextColor="black" value={price} onChangeText={(price) => setPrice(price)} />
        </View>
        <View style={styles.inputView}>
          <TouchableOpacity onPress={pickImage}>
            <Text style={{ color: `black`, fontWeight: 'bold', padding: 11 }}>{imgUrl ? 'Change Photo' : 'Upload Photo'}</Text>
          </TouchableOpacity>
        </View>
        <View style={styles.inputView}>
          <Menu
            visible={catVisible}
            anchor={
              <Text style={{ fontWeight: 'bold', paddingTop: 10 }} onPress={showCatMenu}>
                {categoryId === 1 ? 'Laptop' : categoryId === 2 ? 'Handphone' : categoryId === 3 ? 'PC' : categoryId === 4 ? 'Console' : categoryId === 5 ? 'Peripherals' : 'Pick device category'}
              </Text>
            }
            onRequestClose={hideCatMenu}
            style={{}}
          >
            <MenuItem onPress={() => handleCategorySelection(1)}>Laptop</MenuItem>
            <MenuItem onPress={() => handleCategorySelection(2)}>Handphone</MenuItem>
            <MenuItem onPress={() => handleCategorySelection(3)}>PC</MenuItem>
            <MenuItem onPress={() => handleCategorySelection(4)}>Console</MenuItem>
            <MenuItem onPress={() => handleCategorySelection(5)}>Peripherals</MenuItem>
          </Menu>
        </View>
